...
import org.blitvin.nioserver.ClientContextFactory;
import org.blitvin.nioserver.IOServer;
import org.blitvin.nioserver.MessageProcessor;
import org.blitvin.nioserver.MessageProcessorFactory;

public class IOThroughputTestServer {

	private IOServer<ClientContext,TestMessageProcessor> server;
	private int timeout;
	...
	public IOThroughputTestServer(Properties p) throws IOException {
		this.p = p;
		...
		server = new IOServer<>(port,
				new ClientContextFactory<ClientContext>() {
				...
				new MessageProcessorFactory<ClientContext>() {

					@Override
					public MessageProcessor<ClientContext> newInstance() {
					...
					@Override
					public void reclaimUnused(MessageProcessor<ClientContext> p) {	
					}
				}
				);
	}
	...
	public static void main(String[] args) {
		...
		if (args.length != 1){
			System.err.println("Usage : java org.blitvin.nioserver.IOThroughputTestServer properties_file");
			return;
		}
		try {
		...
		try {
			IOThroughputTestServer testServer = new IOThroughputTestServer(p);
			testServer.test();
		} catch (IOException e) {
...
private static class ClientComThread extends Thread{
		...
		IOThroughputTestServer parent;
		Socket sock;
		int offset;
		int id;
		public ClientComThread(IOThroughputTestServer parent, Socket sock, int offset,int id){
			super();
			this.parent = parent;
			this.sock = sock;
			this.offset = offset;
